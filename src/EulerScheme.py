...
class Differential:
    ...
    def solveII(self, t0:float, N:int, u1:numpy.ndarray=None):
        if u1 is None:
            u1 = next(iter(self.solveI(t0, 2*N)))
        solver_ = EulerII(t0, 2*N, self._euler_function, self._initials, u1)
        class IteratorSolutionII(Iterable):
            ...
            def __next__(self):
                ...
                return value
            def array(self):
                return self.solver.array()[:][::2]
            def result(self):
                return self.solver.array()[0][::2]
...
def test_euler():
    ...
    solution_methods = {
        "без перешагивания"                                 : lambda M: system.solveI(t0, M),
        "с перешагиванием (вторая точка без перешагивания)" : lambda M: system.solveII(t0, M),
        "с перешагиванием (вторая точка из решения)"        : lambda M: system.solveII(t0, M, numpy.array([3.13*numpy.cos(t0/M/2), -3.13*numpy.sin(t0/M/2)]))
    }
    points_amount = {
    ...
    row_:int
    col_: int
    for row_, (points_description, points_amount) in enumerate(points_amount.items()):
        row0 = 2*row_
        row1 = 2*row_ + 1
        time_array = numpy.linspace(0, t0, points_amount)
        u_real = solution(time_array)
        for col_, (solution_description, solution_call) in enumerate(solution_methods.items()):
            col0 = 2 * col_
            col1 = 2 * col_ + 1
            u_calculated = solution_call(points_amount).result()

            axes = plot.axes.add((col0, row_), (col1, row_))
            axes.grid(True)
